...
# -*- coding: utf-8 -*-
"""
FU1 — Poll Trello and send one follow-up email per card.
...
# Send control
SENT_MARKER_TEXT = _get_env("SENT_MARKER_TEXT", default="Sent: FU1")
...
# ----------------- HTTP -----------------
UA = f"TrelloEmailer-FU1/clean (+{FROM_EMAIL or 'no-email'})"
...
def main():
    ...
    for c in cards:
        ...
        _ = choose_id(company, email_v)  # kept for consistency/future use
        ...
        try:
            ...
            processed += 1
            log(f"Sent FU1 to {email_v} — '{title}'")
        except Exception as e:
    ...
    log(f"Done. FU1 emails sent: {processed}")
