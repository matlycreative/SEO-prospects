...
# -*- coding: utf-8 -*-
"""
FU2 — Poll Trello and send one follow-up email per card.

Behavior:
- Reads cards from TRELLO_LIST_ID_FU2
- Sends one email per card (if not already marked)
- Uses hard-coded SUBJECT_A/B and BODY_A/B (no secrets for copy)
- Always uses portfolio link (PORTFOLIO_URL or PUBLIC_BASE/portfolio)
- Marks card with "Sent: FU2" comment to avoid duplicates
"""
...
LINK_TEXT         = _get_env("LINK_TEXT",  default="Our Portfolio")
LINK_COLOR        = _get_env("LINK_COLOR", default="#858585")

# Send control
SENT_MARKER_TEXT = _get_env("SENT_MARKER_TEXT", default="Sent: FU2")
...
# ----------------- HTTP -----------------
UA = f"TrelloEmailer-FU2/clean (+{FROM_EMAIL or 'no-email'})"
SESS = requests.Session()
SESS.headers.update({"User-Agent": UA})

# ----------------- templates (HARDCODED, no free sample) -----------------
SUBJECT_A = "Still relevant for {Company}?"
...
def send_email(to_email: str, subject: str, body_text: str, *, link_url: str, link_text: str, link_color: str):
    ...
    label = (link_text or "Our Portfolio").strip()
...
def main():
    ...
    for c in cards:
        ...
        try:
            ...
            processed += 1
            log(f"Sent FU2 to {email_v} — '{title}'")
        except Exception as e:
    ...
    log(f"Done. FU2 emails sent: {processed}")
