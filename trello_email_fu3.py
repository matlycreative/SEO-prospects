...
Behavior:
- Polls a Trello list (TRELLO_LIST_ID_FU3)
- Sends one email per card if not already marked "Sent: FU3"
- Uses hard-coded SUBJECT_A/B + BODY_A/B (no secrets for copy)
- Always uses portfolio link (PORTFOLIO_URL or PUBLIC_BASE/portfolio)
- Logs and caches sent card IDs to avoid duplicates
"""
...
# Send control (FU3)
SENT_MARKER_TEXT = _get_env("SENT_MARKER_TEXT", default="Sent: FU3")
...
# ----------------- HTTP -----------------
UA = f"TrelloEmailer-FU3/clean (+{FROM_EMAIL or 'no-email'})"
SESS = requests.Session()
SESS.headers.update({"User-Agent": UA})

# ----------------- templates (HARDCODED, no free sample) -----------------
SUBJECT_A = "Before I close this out for {Company}"
...
def signature_html() -> str:
    logo_url = SIGNATURE_LOGO_URL or "https://matlyascend.com/wp-content/uploads/2025/12/cropped-logo-with-ascend-white-e.png"
    if not logo_url:
        return ""
    return """
...
def send_email(to_email: str, subject: str, body_text: str, *, link_url: str, link_text: str, link_color: str):
    ...
    label = (link_text or "My portfolio").strip()
...
def main():
    ...
    for k in ("TRELLO_KEY","TRELLO_TOKEN","FROM_EMAIL","SMTP_PASS","PUBLIC_BASE"):
        if not globals()[k]:
            missing.append(k)
    if not LIST_ID: missing.append("TRELLO_LIST_ID_FU3")
    if missing:
    ...
    for c in cards:
        ...
        try:
            ...
            processed += 1
            log(f"Sent FU3 to {email_v} — '{title}'")
        except Exception as e:
